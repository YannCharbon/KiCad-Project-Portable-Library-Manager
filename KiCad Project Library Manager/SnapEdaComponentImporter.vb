Public Class SnapEdaComponentImporter
    ...
    Private Function GetCompName() As String
        Dim compLibFile = IO.Directory.GetFiles(CompFolder, "*.lib", IO.SearchOption.AllDirectories)(0)
        Dim sr As IO.StreamReader = New IO.StreamReader(compLibFile)
        For Each line In sr.ReadToEnd.Split(vbLf)
    ...
    Private Function _CheckIfFormatIsValid() As Boolean
        ...
        Dim compLibFile = IO.Directory.GetFiles(CompFolder, "*.lib", IO.SearchOption.AllDirectories)

        If compLibFile IsNot Nothing And IO.File.Exists(CompFolder & "\how-to-import.htm") Then
            Return True
        End If

        MsgBox("Component ZIP format does not correspond to SnapEDA archive")
    ...
    Private Sub _FindAndCopyFootprint()
        ...
        Dim files() As String
        files = IO.Directory.GetFiles(CompFolder, "*.kicad_mod", IO.SearchOption.AllDirectories)
        For Each FileName As String In files
            ...
            Dim compFootprintContentSplit = compFootprintContent.Split(vbLf)
            srComp.Close()
            ...
            If footprintContains3dModel = False Then
                compFootprintContentSplit(i - 1) = "  (model ${KIPRJMOD}/Lib/Package3d/" & CompName & ".stp"
                ReDim Preserve compFootprintContentSplit(i + 4)
                compFootprintContentSplit(i) = "    (at (xyz 0 0 0))"
                compFootprintContentSplit(i + 1) = "    (scale (xyz 1 1 1))"
                compFootprintContentSplit(i + 2) = "    (rotate (xyz 0 0 0))"
                compFootprintContentSplit(i + 3) = "  )"
                compFootprintContentSplit(i + 4) = ")"
            End If
    ...
    Private Sub _FindAndCopySymbol()
        ...
        Dim files() As String
        files = IO.Directory.GetFiles(CompFolder, "*.lib", IO.SearchOption.AllDirectories)
